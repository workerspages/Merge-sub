// ===============================================================
// The Final, Corrected, and Complete app.js
// ===============================================================
...
// --- 辅助函数定义 ---
function getSystemUsername() { /* ... 完整逻辑 ... */ }
function generateRandomString() { /* ... 完整逻辑 ... */ }
...
// ===============================================================
// [核心修正] 将特定路由处理器定义在 express.static 之前
// ===============================================================

// --- 认证和首页路由 ---
app.post('/login', async (req, res) => { /* ... 登录逻辑 ... */ });
app.get('/logout', (req, res) => { /* ... 登出逻辑 ... */ });
...
// --- 静态文件服务中间件 (现在放在特定路由之后) ---
// 对于 login.html, css, js 等文件的请求，会在这里被处理
app.use(express.static(path.join(__dirname, 'public')));

// --- 公开的订阅路由 ---
app.get(`/${SUB_TOKEN}`, async (req, res) => { /* ... */ });

// ... (所有其他函数定义，如 generateMergedSubscription, saveData 等保持不变) ...
...
let subscriptions = []; let nodes = ''; const DATA_DIR = process.env.DATA_DIR || path.join(__dirname, 'data'); const DATA_FILE = path.join(DATA_DIR, 'data.json'); const CREDENTIALS_FILE = path.join(DATA_DIR, 'credentials.json'); let credentials = { username: USERNAME, password: PASSWORD };
...
app.get('/get-sub-token', checkAuth, (req, res) => res.json({ token: SUB_TOKEN })); app.get('/get-apiurl', checkAuth, (req, res) => res.json({ ApiUrl: API_URL }));
